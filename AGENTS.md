...
- Classify canonical paths in `int_search_console__pages` so marts stay flat and every consumer sees one definition. Resolve job detail and organization paths to `dim_jobs` and `dim_organizations` so search demand slices by the same dimensions as on-site behavior.
- Resolve job paths only under the current `/jobs/{organization}/{job}/` scheme. Legacy flat single-segment job paths stay classified as `other` rather than being guessed at, matching how legacy job links are treated in the email domain.
...
- Treat `is_job_listing` and `is_job_details` as the export's evidence that Google is parsing `JobPosting` markup. Google only sets them when a page qualified for the jobs experience.

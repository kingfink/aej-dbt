...
- Generate `page_id` from the normalized path in staging and classify pages in `dim_pages` with the shared `get_page_type` macro, so any source that normalizes paths the same way lands on the same page. Parse slugs in staging with `get_organization_slug` and `get_job_slug`, and keep `job_id` and `organization_id` on the facts so they join straight to `dim_jobs` and `dim_organizations`.
...
- Test shared path macros directly against literal cases rather than through a model.

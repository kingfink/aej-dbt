...
- Classify pages in staging with the shared `get_page_type` macro and parse slugs with `get_organization_slug` and `get_job_slug`, so search paths join to site content on the same definitions and future behavioral models such as PostHog can reuse them. Resolve `dim_jobs` and `dim_organizations` in the mart so search demand slices by the same dimensions as on-site behavior.

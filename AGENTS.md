...
- Keep salary observations at job grain in `dim_jobs`, preserving raw salary text and exposing only annual minimum, maximum, and midpoint as salary-specific analytics fields. Compute both annual bounds in `stg_jobs` with the shared `annualize_salary` macro. Check upstream salary validation guarantees with data tests rather than duplicating those checks as model filters; explicit periods win over stored annual inference, and dbt never re-extracts text or invents a period. Store annual bounds as whole-dollar INT64 values and retain missing-pay jobs for coverage denominators.
- Derive salary eligibility from both annual bounds being present. `dim_jobs` exposes `salary_annual_midpoint` as their arithmetic mean so consumers such as Steep reuse one definition. Current salary summaries also filter `is_active`. Median and 25th/75th-percentile midpoint summaries use BigQuery `approx_quantiles` for simplicity, show their observation count and an as-of date, and have no minimum-observation threshold. Distinguish advertised pay from earnings and usable USD pay from disclosure in any currency.
...
- Partition the dedupe window by event timestamp as well as event identifier. Duplicates are exact copies with the same timestamp, so the result is unchanged, but BigQuery can only push an `event_ts` filter through a window whose partition includes it, which lets filters on the staging view prune the day-partitioned source instead of scanning every event.
- Build `fct_web_events` as a standard incremental merge on `event_id` rather than microbatch. Reprocess events that occurred in the three days before the run: recent events land within a day of occurring, and duplicate re-deliveries collapse in the merge.
- Anchor the lookback to `current_timestamp()` rather than to `max(event_ts)` in the target. BigQuery prunes source partitions only for a constant filter; a subquery watermark cannot be pushed through the staging view and scans every event. The cost is that a sync outage longer than the lookback leaves a gap, and backfills older than the lookback also wait, until the weekly full refresh, which also refreshes session, page, job, and organization keys on older events.
...
- Keep analytics persistence and traffic-quality attributes on `dim_web_sessions`. Preserve null as unknown for history collected before a property was captured or when PostHog's query-time virtual classifications are absent from the batch export; never reinterpret unknown as persistent, regular, or human traffic.
- The grain of `fct_web_sessions` is one row per captured PostHog session. Derive active-job engagement from a job page view joined to `fct_jobs_daily` on the event date, keep apply activity as a separate session outcome, and do not encode traffic-quality or geography exclusions in the fact.
...
- Normalize PostHog `page_path` and `session_page_path` in staging with the shared `normalize_page_path` macro, and resolve `page_id` onto `fct_web_events` from `dim_pages`, the dimension Search Console uses. Parse slugs from the normalized path with `get_organization_slug` and `get_job_slug`, preferring the `org_slug` and `job_slug` event properties when present.
...
- Build the Search Console facts as microbatch models on `date_day`, partitioned by month, with `event_time` on the staging models so each batch reads only its own source partitions. BigQuery microbatch requires the partition granularity to match `batch_size`. A date is immutable once Google publishes it, apart from occasional republishes.
- Use monthly batches with a lookback of one. Each batch costs about ten seconds of fixed overhead regardless of size, so daily batches made a full refresh take about thirty minutes per model against the scheduled sync's one hour timeout. Reprocessing the current and previous month on every run is still cheap at this volume, and it covers Google's publish lag: first publishes land within 72 hours of a date closing at the 95th percentile, and republishes arrive up to eight days late.
- Do not set `full_refresh: false` on these models, and keep `begin` at the first exported date. Older batches keep the `job_id` and `organization_id` they resolved when last built, so a job that is renamed or loaded after its searches only resolves on older months when the weekly full refresh rebuilds every batch.
...
- Keep the normalized `page_path` as the natural key in staging, and generate `page_id` and classify pages with the shared `get_page_type` macro in `dim_pages`, the same way `dim_jobs` and `dim_organizations` own their keys. `dim_pages` spans the distinct normalized paths of Search Console and PostHog, unioned and then deduplicated, so search demand and on-site behavior slice by the same page. Facts resolve `page_id` by left joining on `page_path`, so any source that normalizes paths the same way lands on the same page. Parse slugs in staging with `get_organization_slug` and `get_job_slug`, which return a slug only for current-scheme organization and job paths and null for every other path, whether given a repository file path or a site page path. Keep `job_id` and `organization_id` on the facts so they join straight to `dim_jobs` and `dim_organizations`.

...
`parquet_exports.json` continues to publish every column of `dbt_prd.dim_jobs` to `jobs.parquet`. The additive salary interface preserves `salary` and `tags` and adds `salary_min`, `salary_max`, `salary_unit`, `salary_unit_inferred`, `salary_source`, `is_salary_eligible`, `salary_annual_min`, `salary_annual_max`, `salary_annual_midpoint`, `job_level`, and `remote_group`. Existing columns and the export path keep their meanings.

The normalized observations come from reviewed USD base-pay fields in site frontmatter. dbt does not parse salary text or descriptions, infer missing periods, or convert currencies. Explicit periods win; the stored `YEAR` inference is a fallback only, and requires a minimum of at least USD 20,000. Upstream extraction validates finite, positive, ordered bounds, provenance, and periods; staging data tests check these guarantees rather than repeating them as SQL filters. Explicitly unverified jobs are ineligible. A single amount is eligible when its bounds are equal. Broad combined location bands remain eligible and there is no upper salary ceiling.

`stg_jobs` owns annualization, eligibility, and tag groupings; `dim_jobs` exposes these attributes alongside warehouse keys. Annualization uses 2,080 hours, 260 days, 52 weeks, or 12 months and stores each bound as an INT64 rounded to the nearest whole dollar. Original-period amounts retain fractional precision. The midpoint is the arithmetic mean of the annual bounds, retaining half dollars when their sum is odd. Ineligible jobs have null annualized amounts and remain in the dimension so coverage retains its denominator. `job_level` uses the single canonical level tag, or `Unspecified` for missing/conflicting tags; `People Manager` does not override level. `remote_group` is `Remote` only when tagged, otherwise `Other/unspecified`; the original `location` retains geographic restrictions.

Consumers should use these definitions:

- **Current coverage:** count jobs where `is_active = true`; the numerator additionally requires `is_salary_eligible = true`. Label the percentage as usable USD salary coverage, not disclosure in any currency.
- **Pay by level or remote group:** among eligible active jobs, compute the median and 25th/75th percentiles of `salary_annual_midpoint`. Use continuous percentiles, show the salary observation count, and suppress pay summaries below 10 observations. Counts and coverage can still be shown for small groups.

...
The `dbt parse` check installs dbt locally on the GitHub-hosted runner, resolves the locked packages, and parses the project without warehouse credentials. It runs for every pull request, including pull requests from forks, and in parallel with `dbt build` rather than gating it.

The `dbt build` check runs in Modal for pull requests whose branch is in this repository. It is a Slim CI build: it selects project-owned resources modified relative to production and their descendants with `state:modified+,package:this`, and defers every unselected parent to its `dbt_prd` relation with `--defer`. The production state is the manifest of the last successful scheduled production sync, which the sync saves to the `aej-dbt-prod-state` Modal volume and `run_dbt` mounts at `/prod-state`. Until that manifest exists, the check falls back to building every project-owned resource with `package:this`.

Incremental descendants of a modified model have no relation in a new pull request's dataset, so their first CI build covers full history; `fct_search_performance_daily` pays about ten seconds per monthly batch. CI deliberately does not `dbt clone` them from production first. Cloning saved about 30 seconds in testing, but only when a pull request modifies their lineage and only on its first push, and a cloned model recomputes just its incremental window, so CI would stop checking older rows against upstream changes until the weekly full refresh. Revisit cloning, ideally in the same Modal container as the build, if the microbatch backfill makes CI runs that touch it take several minutes. It uses the `ci` target and writes to the pull request's isolated `dbt_ci_<PR number>` dataset. A dbt `on-run-start` hook configures that dataset with a 30-day default table and view expiration before dbt creates relations. BigQuery removes expired relations, while the empty dataset is intentionally retained. Pull requests from forks skip this credentialed check because GitHub does not provide repository secrets to fork workflows.
...
Configure the default-branch ruleset to require the exact check names `Ruff`, `Unit tests`, `dbt parse`, and `dbt build`. Keep the workflow-level dbt trigger unfiltered so required check names are always reported; `state:modified+,package:this` inside the build limits warehouse work to changed project-owned resources, so neither unchanged models nor the installed GA4 package's models are rebuilt.

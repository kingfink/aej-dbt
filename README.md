...
- **Pay by level or remote status:** among active jobs with both annual bounds, derive the midpoint as `(salary_annual_min + salary_annual_max) / 2`, then compute its median and 25th/75th percentiles. Use `level`, `role_type`, and `is_remote` for the breakdowns. Show the salary observation count and suppress pay summaries below 10 observations. Counts and coverage can still be shown for small groups.
